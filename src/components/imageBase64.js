...
async function compressToTarget(file, { targetBytes = DEFAULT_TARGET_BYTES } = {}) {
  ...
  // Start slightly under the target to reduce retries.
  ...
  for (let i = 0; i < 6; i += 1) {
    ...
    // Reduce size budget gradually (but avoid going too low immediately).
...
export async function imageFileToBase64DataUrl(file, { targetBytes = DEFAULT_TARGET_BYTES } = {}) {
  ...
  // Ensure it is a data URL that <img src> can render.
